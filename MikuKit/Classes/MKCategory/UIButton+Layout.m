...
- (void)mk_resetButton_titleLeftImageRight {
     ...
     */
    [self layoutIfNeeded];
    [self setTitleEdgeInsets:UIEdgeInsetsMake(0, - self.imageView.bounds.size.width -5, 0, self.imageView.bounds.size.width + 5)];
    [self setImageEdgeInsets:UIEdgeInsetsMake(0, self.titleLabel.bounds.size.width + 5, 0, - self.titleLabel.bounds.size.width -5)];
}

- (void)mk_resetButtonTopTitleBottomImage {
...
- (void)mk_resetButtonTopImageBottomTitle {
    dispatch_async(dispatch_get_main_queue(), ^{
        // 使图片和文字水平居中显示
        [self setContentHorizontalAlignment:UIControlContentHorizontalAlignmentCenter];
        // 调整文本框位置
...
- (void)mk_resetButtonTopImageBottomTitle_disable {
    dispatch_async(dispatch_get_main_queue(), ^{
        // 使图片和文字水平居中显示
        [self setContentHorizontalAlignment:UIControlContentHorizontalAlignmentCenter];
        //调整文本框位置
